// Verbatim port of Source/WebKitLegacy/Storage/InProcessIDBServer.cpp —
// see the provenance note in BibIDBServer.h. Method bodies are unchanged;
// only the includes, namespace, and dtor differ.
...
{
    // Single-thread adaptation: the original dispatches this teardown to
    // m_queue and blocks on a BinarySemaphore. Here every WorkQueue defers
    // onto the main RunLoop, so blocking the main thread on a queued task
    // would deadlock. There is exactly one thread — tear down inline.
    {
...
{
    // Original asserts !isMainThread() — in this single-threaded build
    // everything IS the main thread; callOnMainThread still defers.
