...
//
// 1. Base rule (priority 1, one per profile): set the profile UA, strip
//    sec-ch-ua* client hints (they'd leak the host browser), and strip
//    Origin/Referer so the extension origin never leaks to target sites when
//    the engine didn't send those headers itself.
...
//    Referer, Origin. Added before the fetch, removed after its terminal
//    event; never static, so a redirect can never ride an engine cookie
//    cross-origin (hops abort under redirect:'error' anyway).
...
export function baseSessionRules(extId, { userAgent }) {
  return [
    {
 ...
 * @param {number} id
 * @param {string} url
 * @param {{cookie?: string, referer?: string, origin?: string}} h
 * @param {string} extId
 */
export function perRequestHeaderRule(id, url, h, extId) {
  const ops = [];
  if (h.cookie != null) ops.push({ header: 'cookie', operation: 'set', value: h.cookie });
  if (h.referer != null) ops.push({ header: 'referer', operation: 'set', value: h.referer });
  if (h.origin != null) ops.push({ header: 'origin', operation: 'set', value: h.origin });
